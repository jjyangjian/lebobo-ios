...
-(recommendView *)recommendV{
    if (!_recommendV) {
        _recommendV = [[recommendView alloc]initWithFrame:CGRectMake(0, self.naviView.bottom + 55, _window_width, _window_height - (self.naviView.bottom + 55+ShowDiff+48)) andNothingImage:[UIImage imageNamed:@"noCart"]];
        if (!_isTabbar) {
            _recommendV.height = _window_height-(self.naviView.bottom+55+ShowDiff);
        }
