...
#import "SWAddressListVC.h"
#import "SWAddressCell.h"
#import "SWAddAddressVC.h"

@interface SWAddressListVC ()<UITableViewDelegate,UITableViewDataSource,SWAddressCellDeleagte>
...
@implementation SWAddressListVC
...
- (void)requestData{
    [SWToolClass getQCloudWithUrl:@"address/list" Suc:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 200) {
            [_dataArray removeAllObjects];
            for (NSDictionary *dic in info) {
                SWAddressModel *model = [[SWAddressModel alloc]initWithDic:dic];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    SWAddressCell *cell = [tableView dequeueReusableCellWithIdentifier:@"addressCELL"];
    if (!cell) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"SWAddressCell" owner:nil options:nil] lastObject];
    }
    cell.delegate = self;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.block) {
        SWAddressModel *model = _dataArray[indexPath.row];
        NSDictionary *dic = @{
            @"id":model.aID,
...
- (void)delateAddress:(SWAddressModel *)model{
    ...
    [SWToolClass postNetworkWithUrl:@"address/del" andParameter:@{@"id":model.aID} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
...
- (void)editAddress:(SWAddressModel *)model{
    SWAddAddressVC *vc = [[SWAddAddressVC alloc]init];
    vc.model = model;
    [[SWMXBADelegate sharedAppDelegate] pushViewController:vc animated:YES];
}
- (void)setDefault:(SWAddressModel *)model{
    [MBProgressHUD showMessage:@""];
    [SWToolClass postNetworkWithUrl:@"address/default/set" andParameter:@{@"id":model.aID} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        [MBProgressHUD hideHUD];
        [MBProgressHUD showError:msg];
