...
- (void)showCoupon:(UIButton *)sender{
    ...
    NSMutableDictionary *mudic = dataArray[sender.tag - 10000];
    id usableCoupon = [mudic valueForKey:@"usableCoupon"];
    NSString *selectID = @"";
    if ([usableCoupon isKindOfClass:[NSDictionary class]] && [usableCoupon count] > 0) {
        selectID = minstr([usableCoupon valueForKey:@"id"]);
    }
    NSDictionary *priceGroup = [mudic valueForKey:@"priceGroup"];
    couponView = [[useCouponView alloc]initWithCouponID:selectID andIsDraw:NO andUsePrice:minstr([priceGroup valueForKey:@"totalPrice"]) andCart:mudic];
    WeakSelf;
    couponView.block = ^(NSDictionary * _Nonnull dic) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if (dic) {
                    [mudic setObject:dic forKey:@"usableCoupon"];
