...
- (void)creatUI{
    [[SWToolClass sharedInstance] lineViewWithFrame:CGRectMake(0, 0, _window_width, 1) andColor:colorf0 andView:self];
    NSArray *array = @[@"客服",@"收藏"];
    for (int i = 0; i < array.count; i ++) {
        ...
        [self addSubview:btn];
        [SWToolClass setUpImgDownText:btn];
    }
    SWBadgeButton *carBtn = [SWBadgeButton buttonWithType:0];
    ...
    [SWToolClass setUpImgDownText:carBtn];
...
- (void)doCar{
    SWCarViewController *vc = [[SWCarViewController alloc]init];
