...
@implementation SWUseCouponCell
...
- (void)setModel:(SWCouponModel *)model{
    ...
    _conditionLabel.text = [NSString stringWithFormat:@"满%@元可用",_model.use_min_price];
...
- (IBAction)doLingqu:(id)sender {
    ...
    [SWToolClass postNetworkWithUrl:@"coupon/receive" andParameter:@{@"couponId":_model.couponID} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 200) {
            _model.is_use = @"1";
            _lingquButton.selected = YES;
            [_lingquButton setBorderColor:RGB_COLOR(@"#d2d2d2", 1)];
        }
    } fail:^{
