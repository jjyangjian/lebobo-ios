...
@implementation SWAnliCell{
    SWImageView *wyShowImgView;
...
- (IBAction)doFollow:(id)sender {
    ...
    if (_model) {
        [SWToolClass postNetworkWithUrl:@"shopsetattent" andParameter:@{@"type":_followBtn.selected ? @"0" : @"1",@"shopid":_model.mer_id} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
            if (code == 200) {
        ...
        if ([_livemodel.isattent isEqual:@"1"]) {
            [SWToolClass postNetworkWithUrl:@"attent/del" andParameter:@{@"touid":_livemodel.uid} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
                _followBtn.userInteractionEnabled = YES;
                if (code == 200) {
            ...
            [SWToolClass postNetworkWithUrl:@"attent/add" andParameter:@{@"touid":_livemodel.uid} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
                if (code == 200) {
...
- (IBAction)doLike:(id)sender {
    ...
    [SWToolClass postNetworkWithUrl:@"kollike" andParameter:@{@"kolid":_model.kolid,@"type":_likeBtn.selected ? @"0" : @"1"} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 200) {
...
- (void)setModel:(SWAnliModel *)model{
    _model = model;
    ...
    _viewsNumLabel.text = [NSString stringWithFormat:@"%@人阅读",_model.views];
    [_likeBtn setTitle:[NSString stringWithFormat:@" %@",_model.likes] forState:0];
    _likeBtn.selected = [_model.islike intValue];
    _contentLabel.attributedText = _model.contentAttStr;
...
- (void)setLivemodel:(SWHomeLiveModel *)livemodel{
    _livemodel = livemodel;
    _contentLabel.attributedText = _livemodel.contentAttStr;
    _showImgViewHeight.constant = _livemodel.imageH;
    _nameLabel.text = _livemodel.nickname;
    [_iconImgVie sd_setImageWithURL:[NSURL URLWithString:_livemodel.avatar]];
    _timeLabel.text = _livemodel.add_time;
    _followBtn.selected = [_livemodel.isattent intValue];
    _viewsNumLabel.text = [NSString stringWithFormat:@"%@人观看",_livemodel.nums];
...
- (void)doBigImage:(UITapGestureRecognizer *)tap{
    ...
    wyShowImgView = [[SWImageView alloc] initWithImageArray:_model?_model.image:_livemodel.thumbs andIndex:index andMine:NO andBlock:^(NSArray * _Nonnull array) {
    }];
