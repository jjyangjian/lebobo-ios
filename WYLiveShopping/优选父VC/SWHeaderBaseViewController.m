...
@interface SWHeaderBaseViewController ()<TUIConversationListControllerListener>

@end

@implementation SWHeaderBaseViewController
- (void)addheaderView{
    ...
    self.messageBtn = [[SWBadgeButton alloc] initWithFrame:CGRectMake(10, 26.5 + statusbarHeight, 35, 35)];
    ...
    self.carBtn = [[SWBadgeButton alloc] initWithFrame:CGRectMake(_window_width-45, 26.5 + statusbarHeight, 35, 35)];
...
//    data.userID = @"2";    // 如果是单聊会话，传入对方用户 ID
//    data.title = @"测试IM";
...
- (void)doCar{
    SWSignInVC *vc = [[SWSignInVC alloc]init];
    [[SWMXBADelegate sharedAppDelegate] pushViewController:vc animated:YES];
}
- (void)doSearch{
    if (_searchBtn.selected) {
        SWGoodsSearchViewController *vc = [[SWGoodsSearchViewController alloc]init];
        [[SWMXBADelegate sharedAppDelegate] pushViewController:vc animated:YES];
    }else{
        SWLiveSearchViewController *vc = [[SWLiveSearchViewController alloc]init];
        [[SWMXBADelegate sharedAppDelegate] pushViewController:vc animated:YES];
    }
}
