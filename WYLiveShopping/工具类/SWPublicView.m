...
+(void)indictorShow {
    ...
    SWPublicView *pubV = [[[NSBundle mainBundle]loadNibNamed:@"SWPublicView" owner:nil options:nil]objectAtIndex:0];
    ...
    for (UIView *view in currentVC.view.subviews) {
        if ([view isKindOfClass:[SWPublicView class]]) {
            have = YES;
            break;
        }
    }
...
+(void)indictorHide {
    ...
    for (UIView *view in currentVC.view.subviews) {
        if ([view isKindOfClass:[SWPublicView class]]) {
            SWPublicView *pubV = (SWPublicView *)view;
...
+(void)publicHide:(UIView *)superView {
    ...
    for (UIView *subv in superView.subviews) {
        if ([subv isKindOfClass:[SWPublicView class]]) {
...
+(void)showImgNoData:(UIView *)superView name:(NSString *)imgName text:(NSString *)text{
     ...
     *  公用类尺寸谨慎修改
     */
    SWPublicView *_noData = [[[NSBundle mainBundle]loadNibNamed:@"SWPublicView" owner:nil options:nil]objectAtIndex:1];
...
+(void)showTextNoData:(UIView *)superView text1:(NSString *)str1 text2:(NSString *)str2 {
     ...
     *  公用类尺寸谨慎修改
     */
    SWPublicView *_noData = [[[NSBundle mainBundle]loadNibNamed:@"SWPublicView" owner:nil options:nil]objectAtIndex:2];
    _noData.frame = CGRectMake(0, 0, superView.width, superView.height);
...
+(void)showCommenting:(UIView *)superView {
    ...
    SWPublicView *_comment = [[[NSBundle mainBundle]loadNibNamed:@"SWPublicView" owner:nil options:nil]objectAtIndex:3];
...
+(void)showCenterGif:(UIView *)superView {
    ...
    SWPublicView *_comment = [[[NSBundle mainBundle]loadNibNamed:@"SWPublicView" owner:nil options:nil]objectAtIndex:4];
