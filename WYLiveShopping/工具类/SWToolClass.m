...
#import "SWToolClass.h"
#import<CommonCrypto/CommonDigest.h>
#import "SWLoginViewController.h"
#import "SWAppDelegate.h"
#import <sys/utsname.h>
//#import ""
@implementation SWToolClass{
    SWSuspensionPlayer *susPlayer;

}
static SWToolClass* kSingleObject = nil;
...
//    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    [[SWToolClass httpManager] GET:requestUrl parameters:nil headers:@{@"Authori-zation":[NSString stringWithFormat:@"Bearer %@",[SWConfig getOwnToken]]} progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
...
+(void)getQCloudNoTokenWithUrl:(NSString *)url Suc:(networkSuccessBlock)successBlock Fail:(networkFailBlock)failBlock{
    ...
    requestUrl = [requestUrl stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
//    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    [[SWToolClass httpManager] GET:requestUrl parameters:nil headers:@{} progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        [MBProgressHUD hideHUD];
        NSLog(@"返回数据：%@",responseObject);
        int code = [[NSString stringWithFormat:@"%@",[responseObject objectForKey:@"status"]] intValue];
        NSDictionary *data = [responseObject valueForKey:@"data"];
...
//    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    NSString *requestUrl = [NSString stringWithFormat:@"%@%@",purl,url];
//    NSString *requestUrl = [purl stringByAppendingFormat:@"/?service=%@&language=",url,[SWConfig canshu]];
    ...
    if ([SWConfig getOwnID].integerValue>0 && [SWConfig getOwnToken]) {
        pDic = [[NSMutableDictionary alloc]initWithObjectsAndKeys:minstr([SWConfig getOwnID]),@"uid",[self getCurrentDeviceModel],@"model",build,@"version",phoneVersion,@"system",package,@"package",[SWConfig getOwnToken],@"token", nil];
    }else{
    ...
    NSLog(@"请求URL:%@",requestUrl);
    NSLog(@"请求参数：%@",pDic);
    [[SWToolClass httpManager] POST:requestUrl parameters:pDic headers:@{@"Authori-zation":[NSString stringWithFormat:@"Bearer %@",[SWConfig getOwnToken]]} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
- (void)showLoginView{
    SWLoginViewController *login = [[SWLoginViewController alloc]init];
    [[SWMXBADelegate sharedAppDelegate] pushViewController:login animated:YES];
}
...
- (void)showSusPlayer:(NSDictionary *)dic andVideoSize:(CGSize)size{
    ...
    susPlayer = [[SWSuspensionPlayer alloc]initWithFrame:CGRectMake(0, 0, _window_width*0.3, hhhhhh) andRoomMessage:dic];
