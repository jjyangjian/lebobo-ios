...
- (void)getRowHeight{
    CGFloat contentH = [[SWToolClass sharedInstance] heightOfString:_comment andFont:SYS_Font(15) andWidth:_window_width-30];
    if (_pics.count == 0) {
        _picsH = 0;
        _rowH = 100 + contentH;
    }else{
    ...
    if (self.merchant_reply_content.length > 0) {
        CGFloat hhh = [[SWToolClass sharedInstance] getSpaceLabelHeightwithSpeace:5 withFont:SYS_Font(13) withWidth:_window_width-60 andString:[self.merchant_reply_content string]];
        _rowH += (hhh + 35);
    }
}
@end
