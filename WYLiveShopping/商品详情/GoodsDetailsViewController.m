...
@interface GoodsDetailsViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,WKNavigationDelegate,UIScrollViewDelegate,WMPlayerDelegate>{
    ...
    UILabel *storeMioashuL;
    UILabel *storeFuwuL;
    UILabel *storeWuliuL;
    UILabel *storeTypeView;
...
- (void)creatGoodsMessage{
    ...
    [_goodsView addSubview:couponView];
    [couponView mas_makeConstraints:^(MASConstraintMaker *make) {
       make.left.width.equalTo(_goodsView);
       make.top.equalTo(integralView.mas_bottom);
...
- (void)showCouponView{
    if (!couponDrawView) {
        couponDrawView = [[useCouponView alloc]initWithCouponID:_goodsID andIsDraw:YES andUsePrice:@"" andCart:@{}];
        [self.view addSubview:couponDrawView];
    }else{
        [couponDrawView show];
    }
}
...
- (UIView *)storeView{
    if (!_storeView) {
        ...
        [storeNameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.centerY.equalTo(storeThumbImgView).multipliedBy(0.8);
        }];
        ...
        [view mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.equalTo(storeThumbImgView.mas_bottom).offset(15);
            make.right.equalTo(_storeView).offset(-10);
...
- (void)doStoreHome{
    WYStoreHomeTbabarViewController *vc = [[WYStoreHomeTbabarViewController alloc]initWithID:mer_id];
//    vc.mer_id = mer_id;
    [[MXBADelegate sharedAppDelegate] pushViewController:vc animated:YES];
}
#pragma mark -- 推荐视频
    ...
    [WYToolClass getQCloudWithUrl:[NSString stringWithFormat:@"product/detail/%@",_goodsID] Suc:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 200) {
            ...
            if ([mer_id integerValue] > 0) {
                ...
                mer_name = minstr([info valueForKey:@"shop_name"]);
                ...
                storeMioashuL.attributedText = [self setStoreAttMessage:[NSString stringWithFormat:@"宝贝描述 %@",minstr([info valueForKey:@"shop_score1"])] andColor:color64];
                storeFuwuL.attributedText = [self setStoreAttMessage:[NSString stringWithFormat:@"商家服务 %@",minstr([info valueForKey:@"shop_score2"])] andColor:normalColors];
                storeWuliuL.attributedText = [self setStoreAttMessage:[NSString stringWithFormat:@"物流服务 %@",minstr([info valueForKey:@"shop_score3"])] andColor:color64];
...
//                }
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    if ([mer_id isEqual:[Config getOwnID]]) {
                        [MBProgressHUD showError:@"不能与自己发送私信"];
                        return;
                    }

                    TUIConversationCellData *data = [[TUIConversationCellData alloc] init];
                    data.userID = mer_id;    // 如果是单聊会话，传入对方用户 ID
                    data.title = mer_name;    // 如果是单聊会话，传入对方用户 ID
...
- (void)doLive{
    if (_liveArray.count == 1) {
        ...
        WYLiveForGoodsMoreViewController *vc = [[WYLiveForGoodsMoreViewController alloc]init];
        vc.productid = _goodsID;
        [[MXBADelegate sharedAppDelegate] pushViewController:vc animated:YES];
    }
}
- (void)checkLive:(NSDictionary *)roomdic{
    [WYToolClass postNetworkWithUrl:@"live/check" andParameter:@{@"stream":minstr([roomdic valueForKey:@"stream"])} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        [MBProgressHUD hideHUD];
        if (code == 200) {
