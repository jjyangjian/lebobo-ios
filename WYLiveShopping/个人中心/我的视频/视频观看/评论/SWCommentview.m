...
#import "SWPublicView.h"
#import "SWCommentModel.h"
#import <HPGrowingTextView/HPGrowingTextView.h>
#import "SWTwEmojiView.h"
#import "SWCommCell.h"

@interface SWCommentview ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate,SWCommCellDelegate,HPGrowingTextViewDelegate,SWTwEmojiViewDelegate>
{
...
- (void)reloadCurCell:(SWCommentModel *)model andIndex:(NSIndexPath *)curIndex andReplist:(NSArray *)list{
    for (int i = 0; i < _itemsarray.count; i++) {
        ...
        if ([minstr([muDic valueForKey:@"id"]) isEqual:model.parentid]) {
            NSDictionary *dddd = @{
                @"ismore":[NSString stringWithFormat:@"%d",model.isMore],
                @"list":list
            };
            [muDic setObject:dddd forKey:@"replay"];
//            mol.replyList = list;
//            NSLog(@"-------%@",mol.replyList);
...
-(void)reloaddata:(NSString *)from{
    ...
    WeakSelf;
    [SWToolClass getQCloudWithUrl:[NSString stringWithFormat:@"videocomment&vid=%@&cid=0&lastid=%@",_videoid,lastid] Suc:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 200) {
            //隐藏评论加载中
            [SWPublicView hideCommenting:weakSelf.tableview];
            NSArray *commentlist = [info valueForKey:@"list"];

//            int allcomments = [[NSString stringWithFormat:@"%@",[info valueForKey:@"comments"]] intValue];
//            weakSelf.allCommentLabels.text = [NSString stringWithFormat:@"%d %@",allcomments,@"评论"];
//            self.talkCount([NSString stringWithFormat:@"%@",[info valueForKey:@"comments"]] );
            ...
            if (_itemsarray.count == 0) {
                ...
                lastid = minstr([[_itemsarray lastObject] valueForKey:@"id"]);
            }
...
//        _modelarray = [NSMutableArray array];
        ...
        _allCommentLabels.text = [NSString stringWithFormat:@"%@（%d） ",@"评论",allcomments];
        ...
        //评论加载中
//        [SWPublicView showCommenting:_tableview];
...
- (void)growingTextViewDidChange:(HPGrowingTextView *)growingTextView {
    ...
    if (newText.length < 1) {
        ...
        NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:textView.text              attributes:@{NSForegroundColorAttributeName:color32,NSFontAttributeName:[UIFont systemFontOfSize:16]}];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([_touid isEqual:[SWConfig getOwnID]]) {
    ...
    [_textField becomeFirstResponder];
    NSString *path = [NSString stringWithFormat:@"回复给:%@",[subdic valueForKey:@"nickname"]];
...
//    SWCommentModel *model = _modelarray[indexPath.row];
//    return model.rowH;
//}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    [self.tableview deselectRowAtIndexPath:indexPath animated:NO];
    SWCommCell *cell = [tableView dequeueReusableCellWithIdentifier:@"commCELL"];
    if (!cell) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"SWCommCell" owner:nil options:nil] lastObject];
    }
    cell.model = [[SWCommentModel alloc]initWithDic:_itemsarray[indexPath.row]];
    cell.delegate = self;
...
-(void)getNewCount:(int)counts{
     _allCommentLabels.text = [NSString stringWithFormat:@"%d 评论",counts];
}
...
-(void)pushmessage{
    ...
    NSString *url = [purl stringByAppendingFormat:@"/?service=Video.setComment&videoid=%@&content=%@&uid=%@&token=%@&touid=%@&commentid=%@&parentid=%@&at_info=%@",_videoid,path,[SWConfig getOwnID],[SWConfig getOwnToken],sendtouid,sendcommentid,sendparentid,at_json];
    ...
    [SWToolClass postNetworkWithUrl:@"videosetcommnet" andParameter:@{
        ...
        if (code == 200) {
            ...
            //刷新评论数
            int allcomments = [[NSString stringWithFormat:@"%@",[info valueForKey:@"comments"]] intValue];
            weakSelf.allCommentLabels.text = [NSString stringWithFormat:@"%@（%d） ",@"评论",allcomments];
            self.talkCount([NSString stringWithFormat:@"%@",[info valueForKey:@"comments"]] );
        }else{
             [MBProgressHUD showError:msg];
        }
...
-(void)pushDetails:(NSDictionary *)commentdic{
    ...
    _touid = [NSString stringWithFormat:@"%@",[commentdic valueForKey:@"uid"]];
    if ([_touid isEqual:[SWConfig getOwnID]]) {
        return;
    }
    [_textField becomeFirstResponder];
    NSString *path = [NSString stringWithFormat:@"回复给:%@",[commentdic valueForKey:@"nickname"]];
    _textField.placeholder = path;
    _parentid = [NSString stringWithFormat:@"%@",[commentdic valueForKey:@"id"]];
    _commentid = [NSString stringWithFormat:@"%@",[commentdic valueForKey:@"cid"]];
    isReply = YES;
}
