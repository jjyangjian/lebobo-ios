...
@implementation SWCommDetailCell
...
- (void)setModel:(SWDetailmodel *)model{
    _model = model;
//    _nameL.text = _model.user_nicename;
    ...
    [attStr addAttributes:@{NSForegroundColorAttributeName:color96} range:NSMakeRange(0, _model.user_nicename.length)];

    if ([_model.toname length] > 0) {
        [attStr addAttributes:@{NSForegroundColorAttributeName:color96} range:NSMakeRange(_model.user_nicename.length+2, _model.toname.length)];
    }
