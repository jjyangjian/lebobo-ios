...
#import "SWCommCell.h"
#import "SWCommDetailCell.h"
#import "SWDetailmodel.h"
#import "SWPublicObj.h"
@implementation SWCommCell{
    NSString *lastid;
}
...
- (void)setModel:(SWCommentModel *)model{
    ...
    _zanNumLabel.text = _model.likes;
    if ([_model.islike isEqual:@"1"]) {
        [_zanBtn setImage:[UIImage imageNamed:@"likecomment-click"] forState:0];
        _zanNumLabel.textColor = RGB_COLOR(@"#fa561f", 1);
    }else{
        [_zanBtn setImage:[UIImage imageNamed:@"likecomment"] forState:0];
        _zanNumLabel.textColor = RGB(130, 130, 130);
    }
    //匹配表情文字
    NSArray *resultArr  = [[SWToolClass sharedInstance] machesWithPattern:emojiPattern andStr:_model.content];
    if (!resultArr) return;
...
//    [attstr appendAttributedString:dateStr];
    ...
    if ([_model.replys intValue] > 0) {
        ...
        for (NSDictionary *dic in _replyArray) {
            SWDetailmodel *model = [[SWDetailmodel alloc]initWithDic:dic];
            HHHH += model.rowH;
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    SWCommDetailCell *cell = [tableView dequeueReusableCellWithIdentifier:@"commDetailCELL"];
    if (!cell) {
        cell = [[SWCommDetailCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"SWCommDetailCell"];
        
    }

    SWDetailmodel *model = [[SWDetailmodel alloc]initWithDic:_replyArray[indexPath.row]];
    cell.model = model;
    return cell;
}
...
- (void)requestData{
    [SWToolClass getQCloudWithUrl:[NSString stringWithFormat:@"videocomment&vid=%@&cid=%@&lastid=%@",_model.videoID,_model.parentid,lastid] Suc:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 200) {
//            if ([info count]>0) {
...
- (IBAction)zanBtnClick:(id)sender {
    ...
    if ([_model.ID isEqual:[SWConfig getOwnID]]) {
...
//    NSString *url = [purl stringByAppendingFormat:@"?service=Video.addCommentLike&uid=%@&commentid=%@&token=%@",[SWConfig getOwnID],_model.parentid,[SWConfig getOwnToken]];
    [SWToolClass postNetworkWithUrl:@"videocommentlike" andParameter:@{@"cid":_model.parentid,@"type":[_model.islike isEqual:@"1"] ? @"0" : @"1"} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 200) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [_zanBtn.imageView.layer addAnimation:[SWPublicObj bigToSmallRecovery] forKey:nil];
            });
