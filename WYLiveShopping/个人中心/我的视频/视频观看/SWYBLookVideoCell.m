...
@implementation SWYBLookVideoCell
...
- (UIView *)rightView{
    if (!_rightView) {
        ...
        _likebtn = [SWPublicObj setUpImgDownText:_likebtn];
        ...
        _commentBtn = [SWPublicObj setUpImgDownText:_commentBtn];
        ...
        _enjoyBtn = [SWPublicObj setUpImgDownText:_enjoyBtn];
...
- (void)setDataDic:(NSDictionary *)dataDic {
    ...
    if (![SWPublicObj checkNull:minstr([_dataDic valueForKey:@"anyway"])]) {
    ...
    [_followBtn.layer addAnimation:[SWPublicObj followShowTransition] forKey:nil];
...
-(void)guanzhuzhubo
{
    ...
    if ([minstr([_dataDic valueForKey:@"isattent"]) isEqual:@"1"]) {
        [SWToolClass postNetworkWithUrl:@"attent/del" andParameter:@{@"touid":videoUserID} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
            _followBtn.userInteractionEnabled = YES;
            if (code == 200) {
        ...
        [SWToolClass postNetworkWithUrl:@"attent/add" andParameter:@{@"touid":videoUserID} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
            if (code == 200) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [_followBtn.imageView.layer addAnimation:[SWPublicObj smallToBigToSmall] forKey:nil];
 ...
 -(void)dolike{
     ...
     [SWToolClass postNetworkWithUrl:@"videolike" andParameter:subdic success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
         if(code == 200) {
...
//        center.userID =[SWConfig getOwnID];
//    }else{
//        center.userID =_hostid;
//    }
//    center.hidesBottomBarWhenPushed = YES;
//    [[SWMXBADelegate sharedAppDelegate] pushViewController:center animated:YES];
