...
@interface SWYBLookVideoVC ()<UICollectionViewDelegate,UICollectionViewDataSource,HPGrowingTextViewDelegate,SWTwEmojiViewDelegate>
{
...
@property (nonatomic, strong) SWYBVideoControlView *controlView;
@property(nonatomic,strong)SWYBLookVideoCell *playingCell;

@end

@implementation SWYBLookVideoVC
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (_controlView.playButton.hidden == NO) {
        return;
    }
//    [_controlView controlSingleTapped];
...
//            SWYBLookVideoCell *disCell = (SWYBLookVideoCell*)[self.collectionView cellForItemAtIndexPath:indexPath];
...
- (SWYBVideoControlView *)controlView {
    if (!_controlView) {
        _controlView = [SWYBVideoControlView new];
        @weakify(self);
...
- (SWYBLookVideoCell *)playingCell {
    _playingCell = (SWYBLookVideoCell*)[_collectionView cellForItemAtIndexPath:self.player.playingIndexPath];
    return _playingCell;
}
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[SWYBLookVideoCell class] forCellWithReuseIdentifier:reuseIdentifier];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    WeakSelf;
    SWYBLookVideoCell *cell = (SWYBLookVideoCell *)[collectionView dequeueReusableCellWithReuseIdentifier:reuseIdentifier forIndexPath:indexPath];
...
-(void)startWatchVideo{
    NSDictionary *dic = @{
        @"vid":_videoID,
        @"sign":[SWToolClass sortString:@{@"vid":_videoID}]
    };
    [SWToolClass postNetworkWithUrl:@"setPlays" andParameter:dic success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
...
//    }
    WeakSelf;
    [SWToolClass postNetworkWithUrl:@"videosetcommnet" andParameter:@{
            @"vid":_videoID,
            @"cid":@"0",
            @"pid":@"0",
            @"touid":@"0",
        ...
        if(code == 200){
            ...
            SWYBLookVideoCell *disCell = (SWYBLookVideoCell*)[self.collectionView cellForItemAtIndexPath:_lastPlayIndexPath];
...
-(void)morebtn:(id)sender {
    if (!_videomore) {
        ...
        _videomore = [[SWVideoMoreView alloc]initWithFrame:CGRectMake(0, _window_height+20, _window_width, hh) andHostDic:_currentVideoDic cancleblock:^(id array) {
...
//            [weakSelf.controlView.share setTitle:[NSString stringWithFormat:@"%@",weakSelf.shares] forState:0];
    ...
    //            jubao.fromWhere = weakSelf.fromWhere ? weakSelf.fromWhere:@"SWLookVideo";
                [[SWMXBADelegate sharedAppDelegate] pushViewController:jubao animated:YES];
            });
        };
...
- (void)doAddShre:(NSString *)shares{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        SWYBLookVideoCell *cell = (SWYBLookVideoCell *)[_collectionView cellForItemAtIndexPath:_lastPlayIndexPath];
        cell.dataDic = muDic;
    });
}
- (void)showGoodsDeatile{
    ...
    goodsDView = [[SWLookVGoodsDView alloc]initWithGoodsMsg:[[dic valueForKey:@"goods"] firstObject]];
