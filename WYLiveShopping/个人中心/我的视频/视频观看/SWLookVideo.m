...
#import "SWLookVideo.h"

#import <CWStatusBarNotification/CWStatusBarNotification.h>
#import "SWVideoMoreView.h"
...
#import "SWVideoPauseView.h"
#import "SWJubaoVC.h"

#import "SWJPLookProgressView.h"
#import "SWJPBufferView.h"

#import "SWFrontView.h"
#import <HPGrowingTextView/HPGrowingTextView.h>
#import <AFNetworking/AFNetworking.h>
#import "SWTwEmojiView.h"
#import "SWPublicObj.h"
@interface SWLookVideo ()<UIActionSheetDelegate,UIScrollViewDelegate,videoPauseDelegate,JPVideoPlayerDelegate,HPGrowingTextViewDelegate,SWTwEmojiViewDelegate>
{
    CWStatusBarNotification *_notification;
    
    SWVideoPauseView *pasueView;
...
@property(nonatomic,strong)SWVideoMoreView *videomore;                 //分享view

@property(nonatomic,strong)UIButton *goBackBtn;
@property(nonatomic,strong)UIButton *goBackShadow;

@property(nonatomic,strong)SWCommentview *comment;                     //评论
...
@property(nonatomic,strong)SWFrontView *currentFront;                 //展现在前台的View（first、second、thirdFront）
@property(nonatomic,strong)UIImageView *bufferIV;                     //首次切换过度封面图（消除闪屏）
@property (nonatomic, strong) UIImageView *firstImageView;
@property(nonatomic,strong) SWFrontView *firstFront;
@property (nonatomic, strong) UIImageView *secondImageView;
@property(nonatomic,strong) SWFrontView *secondFront;
@property (nonatomic, strong) UIImageView *thirdImageView;
@property(nonatomic,strong) SWFrontView *thirdFront;

@property(nonatomic, assign) CGFloat scrollViewOffsetYOnStartDrag;

@end

@implementation SWLookVideo
...
//    NSString *sign_del = [SWConfig getSignOfDelVideo];
//    if ([sign_del isEqual:@"1"]&&!_fromWhere) {
//        [SWConfig saveSignOfDelVideo:@"0"];
...
//            if (haveAnmation) {
//                if ([[SWConfig getOwnID] isEqual:weakSelf.hostid] || [[info valueForKey:@"isattent"] isEqual:@"1"]) {
...
//                [frontView.followBtn.layer addAnimation:[SWPublicObj followShowTransition] forKey:nil];
//            }
//
//        }else if ([code isEqual:@"700"]) {
//            [SWPublicObj tokenExpired:[data valueForKey:@"msg"]];
//        }else{
...
-(void)setVideoData:(NSDictionary *)videoDic withFront:(SWFrontView*)front{
    ...
    front.likebtn = [SWToolClass setUpImgDownText:front.likebtn];
    [front.enjoyBtn setTitle:[NSString stringWithFormat:@"%@",weakSelf.shares] forState:0];
    front.enjoyBtn = [SWToolClass setUpImgDownText:front.enjoyBtn];
    [front.commentBtn setTitle:[NSString stringWithFormat:@"%@",weakSelf.comments] forState:0];
    front.commentBtn = [SWToolClass setUpImgDownText:front.commentBtn];
    
    if ([[SWConfig getOwnID] isEqual:weakSelf.hostid] || [isattent isEqual:@"1"]) {
    ...
    [front.followBtn.layer addAnimation:[SWToolClass followShowTransition] forKey:nil];
}
-(void)setUserData:(NSDictionary *)dataDic withFront:(SWFrontView*)front{
...
//    if ([[SWConfig getOwnID] isEqual:dataUid]) {
//        front.followBtn.hidden = YES;
//    }else{
//        front.followBtn.hidden = NO;
//    }
...
-(void)changeRoom{
    ...
    [_currentPlayerIV jp_playVideoMuteWithURL:[NSURL URLWithString:_playUrl]
                               bufferingIndicator:[SWJPBufferView new]
                                     progressView:[SWJPLookProgressView new]
...
-(void)videoStart {
    if ([_hostid isEqual:[SWConfig getOwnID]]) {
        return;
    }
    NSString *random_str = [SWPublicObj stringToMD5:[NSString stringWithFormat:@"%@-%@-#2hgfk85cm23mk58vncsark",[SWConfig getOwnID],_videoid]];
    NSString *url = [purl stringByAppendingFormat:@"?service=Video.addView&uid=%@&token=%@&videoid=%@&random_str=%@",[SWConfig getOwnID],[SWConfig getOwnToken],_videoid,random_str];
//    [YBNetworking postWithUrl:url Dic:nil Suc:^(NSDictionary *data, NSString *code, NSString *msg) {
...
//    NSString *random_str = [SWPublicObj stringToMD5:[NSString stringWithFormat:@"%@-%@-#2hgfk85cm23mk58vncsark",[SWConfig getOwnID],_videoid]];
//    NSString *url = [purl stringByAppendingFormat:@"?service=Video.setConversion&uid=%@&token=%@&videoid=%@&random_str=%@",[SWConfig getOwnID],[SWConfig getOwnToken],_videoid,random_str];
//    [YBNetworking postWithUrl:url Dic:nil Suc:^(NSDictionary *data, NSString *code, NSString *msg) {
...
-(void)pubAnimation:(SWFrontView*)front{
    //音乐
    front.symbolAIV.hidden = NO;
    [front.discIV.layer addAnimation:[SWPublicObj rotationAnimation] forKey:@"rotation"];
    [front.symbolAIV.layer addAnimation:[SWPublicObj caGroup] forKey:nil];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        front.symbolBIV.hidden = NO;
        [front.symbolBIV.layer addAnimation:[SWPublicObj caGroup] forKey:nil];
    });
...
//点击关注主播
-(void)guanzhuzhubo{
    if ([[SWConfig getOwnID] intValue]<0) {
        [SWPublicObj warnLogin];
        return;
    }
...
//                    [_currentFront.followBtn.imageView.layer addAnimation:[SWPublicObj smallToBigToSmall] forKey:nil];
...
-(void)doPauseView{
    ...
    if (!pasueView) {
        ...
        pasueView = [[SWVideoPauseView alloc]initWithFrame:CGRectMake(_window_width/2-40, pauseVH, 80, 80) andVideoMsg:_hostdic];
...
-(void)morebtn:(id)sender {
    ...
    if (!_videomore) {
        WeakSelf;
        NSArray *array = @[];//[SWCommon share_type];
        CGFloat hh = _window_height/3+30+ShowDiff;
        if (array.count == 0) {
            hh = _window_height/3/2+30+ShowDiff;
        }
        _videomore = [[SWVideoMoreView alloc]initWithFrame:CGRectMake(0, _window_height+20, _window_width, hh) andHostDic:_hostdic cancleblock:^(id array) {
        ...
        _videomore.jubaoBlock = ^(id array) {
            SWJubaoVC *jubao = [[SWJubaoVC alloc]init];
//            jubao.dongtaiId = array;
//            jubao.fromWhere = weakSelf.fromWhere ? weakSelf.fromWhere:@"SWLookVideo";
...
- (void)checkLiveSuc:(NSString *)isFull{
    ...
    NSString *url = [purl stringByAppendingFormat:@"?service=Video.addLike&uid=%@&videoid=%@&type=%@&token=%@",[SWConfig getOwnID],_videoid,isFull,[SWConfig getOwnToken]];
...
//    }
    ...
    WeakSelf;
    if (!_comment) {
        _comment = [[SWCommentview alloc]initWithFrame:CGRectMake(0,_window_height, _window_width, _window_height) hide:^(NSString *type) {
            [UIView animateWithDuration:0.3 animations:^{
...
-(void)pushmessage{
    ...
    NSString *url = [purl stringByAppendingFormat:@"/?service=Video.setComment&videoid=%@&content=%@&uid=%@&token=%@&touid=%@&commentid=%@&parentid=%@&at_info=%@",_videoid,path,[SWConfig getOwnID],[SWConfig getOwnToken],_hostid,@"0",@"0",at_json];
...
-(UIScrollView *)backScrollView{
    if (!_backScrollView) {
        ...
        WeakSelf;
        _firstFront = [[SWFrontView alloc]initWithFrame:_firstImageView.frame callBackEvent:^(NSString *type) {
            [weakSelf clickEvent:type];
        }];
        [_backScrollView addSubview:_firstFront];
        
        _secondFront = [[SWFrontView alloc]initWithFrame:_secondImageView.frame callBackEvent:^(NSString *type) {
            [weakSelf clickEvent:type];
        }];
        [_backScrollView addSubview:_secondFront];
        
        _thirdFront = [[SWFrontView alloc]initWithFrame:_thirdImageView.frame callBackEvent:^(NSString *type) {
            [weakSelf clickEvent:type];
        }];
...
-(void)showtextfield{
    ...
    if (!_emojiV) {
        _emojiV = [[SWTwEmojiView alloc]initWithFrame:CGRectMake(0, _window_height, _window_width, EmojiHeight+ShowDiff)];
...
//        goodsDView = nil;
//    }
//    goodsDView = [[SWLookVGoodsDView alloc]initWithGoodsMsg:[_hostdic valueForKey:@"goodsinfo"]];
