...
@interface SWVideoMoreView ()<UICollectionViewDelegate,UICollectionViewDataSource>
{
...
@end
@implementation SWVideoMoreView
-(instancetype)initWithFrame:(CGRect)frame andHostDic:(NSDictionary *)dic cancleblock:(xinxinblock)block delete:(xinxinblock)deleteblock share:(xinxinblock)share{
    ...
    if (self) {
        ...
        _itemarray = [NSArray arrayWithArray:[SWCommon share_type]];
        ...
        shareL.text = [NSString stringWithFormat:@"分享至"];
...
//        NSArray *array = @[@"举报",@"复制",YZMsg(@"拉黑"),YZMsg(@"保存")];
        NSArray *array = @[@"复制链接",@"保存"];

        if ([minstr([_hostdic valueForKey:@"uid"]) isEqual:[SWConfig getOwnID]]) {
            array =@[@"复制链接",@"删除",@"保存"];
        }
        //如果没有分享
...
-(void)makeaction:(UIButton *)sender{
    if ([sender.titleLabel.text isEqual:@"拉黑"]) {
        [self setBlack:sender];
    }else if ([sender.titleLabel.text isEqual:@"举报"]) {
        [self jubao];
    }else if ([sender.titleLabel.text isEqual:@"复制链接"]) {
        UIPasteboard *paste = [UIPasteboard generalPasteboard];
        paste.string = [NSString stringWithFormat:@"%@",[_hostdic valueForKey:@"url"]];
        [MBProgressHUD showSuccess:@"复制成功"];
    }else if ([sender.titleLabel.text isEqual:@"保存"]) {
        [self downloadVideo:sender];
    }else if ([sender.titleLabel.text isEqual:@"删除"]) {
        ...
        UIAlertAction *cnacelA = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
...
-(void)downloadVideo:(UIButton *)sender{
    ...
    NSString *href = [NSString stringWithFormat:@"%@",[_hostdic valueForKey:@"url"]];
    if (href.length < 2) {
        return;
    }
...
- (void)video:(NSString *)videoPath didFinishSavingWithError:(NSError *)error contextInfo: (void *)contextInfo {
    if (error == nil) {
    NSLog(@"视频保存成功");
        hud.detailsLabel.text = @"视频保存成功";
    }else{
    NSLog(@"视频保存失败");
        hud.detailsLabel.text = @"视频保存失败";
    }
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [MBProgressHUD hideHUDForView:self animated:YES];
    });
...
//
//        if (titles.length == 0) {
//            titles = [SWCommon video_share_title];
//        }
//
        ParamsURL = [NSURL URLWithString:[h5url stringByAppendingFormat:@"/appapi/video/index?id=%@",[_hostdic valueForKey:@"id"]]]; 
        [shareParams SSDKSetupShareParamsByText:@""
                                         images:[_hostdic valueForKey:@"thumb"]
                                            url:ParamsURL
                                          title:minstr([_hostdic valueForKey:@"name"])
                                           type:SSDKContentType];
    
    __weak SWVideoMoreView *weakself = self;
    //进行分享
...
-(void)setBlack:(UIButton *)sender{
    sender.userInteractionEnabled = NO;
    NSString *url = [purl stringByAppendingFormat:@"?service=Video.setBlack&uid=%@&token=%@&videoid=%@",[SWConfig getOwnID],[SWConfig getOwnToken],[NSString stringWithFormat:@"%@",[_hostdic valueForKey:@"id"]]];
    WeakSelf;
...
-(void)deletemovie:(UIButton *)sender{
     sender.userInteractionEnabled = NO;
    WeakSelf;
    [SWToolClass postNetworkWithUrl:@"videodel" andParameter:@{@"vid":minstr([_hostdic valueForKey:@"id"])} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
            if (code == 200) {
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:@"videoDelete" object:dic userInfo:nil];
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    weakSelf.deleteblock(videoIDStr);
                });
            }else{
...
-(void)addShare{
    WeakSelf;
    NSDictionary *dic = @{
        @"vid":minstr([_hostdic valueForKey:@"id"]),
        @"sign":[SWToolClass sortString:@{@"vid":minstr([_hostdic valueForKey:@"id"])}]
    };
