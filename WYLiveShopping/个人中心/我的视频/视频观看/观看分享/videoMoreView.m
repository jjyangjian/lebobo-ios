...
//        NSArray *array = @[@"举报",@"复制",YZMsg(@"拉黑"),YZMsg(@"保存")];
        ...
        if ([minstr([_hostdic valueForKey:@"uid"]) isEqual:[Config getOwnID]]) {
            array =@[@"复制链接",@"删除",@"保存"];
        }
        //如果没有分享
...
-(void)deletemovie:(UIButton *)sender{
     sender.userInteractionEnabled = NO;
    WeakSelf;
    [WYToolClass postNetworkWithUrl:@"videodel" andParameter:@{@"vid":minstr([_hostdic valueForKey:@"id"])} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
            if (code == 200) {
...
-(void)addShare{
    ...
    NSDictionary *dic = @{
        ...
        @"sign":[WYToolClass sortString:@{@"vid":minstr([_hostdic valueForKey:@"id"])}]
    };
