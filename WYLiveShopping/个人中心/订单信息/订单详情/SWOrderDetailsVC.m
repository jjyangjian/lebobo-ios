...
@interface SWOrderDetailsVC ()<UITextViewDelegate>
@property (nonatomic, strong) UIScrollView *backScrollView;
@property (nonatomic, strong) UIView *bottomView;
@property (nonatomic, strong) NSMutableArray *goodsArray;
@property (nonatomic, strong) SWPayTypeSelectView *payTypeView;
@property (nonatomic, strong) UIView *sendGiftView;

@end

@implementation SWOrderDetailsVC
...
- (void)viewDidLoad {
    ...
    if (self.orderType == 0) {
        ...
        self.backScrollView.height = _window_height - self.naviView.bottom;
    }
}
- (void)creatUI{
    ...
    UIView *SWAddressView = [[UIView alloc]initWithFrame:CGRectMake(0, progressView.bottom + 10, _window_width, 70)];
    SWAddressView.backgroundColor = [UIColor whiteColor];
    [_backScrollView addSubview:SWAddressView];
    ...
    [SWAddressView addSubview:adsNameL];
    [adsNameL mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(SWAddressView).offset(15);
        make.bottom.equalTo(SWAddressView.mas_centerY).offset(-3);
    }];
    ...
    [SWAddressView addSubview:adsContL];
    ...
    UIImageView *lineImgV = [[UIImageView alloc]initWithFrame:CGRectMake(0, SWAddressView.height-1, _window_width, 1)];
    lineImgV.image = [UIImage imageNamed:@"地址线"];
    lineImgV.contentMode = UIViewContentModeScaleAspectFill;
    [SWAddressView addSubview:lineImgV];
    
    UIView *shopView = [[UIView alloc]initWithFrame:CGRectMake(0, SWAddressView.bottom + 10, _window_width, 80 + self.goodsArray.count * 80)];
    ...
    [[SWToolClass sharedInstance] lineViewWithFrame:CGRectMake(15, 39, _window_width-30, 1) andColor:colorf0 andView:shopView];
    for (int i = 0; i < self.goodsArray.count; i ++) {
        ...
        SWCartModel *model = self.goodsArray[i];
    ...
    [[SWToolClass sharedInstance] lineViewWithFrame:CGRectMake(15, shopView.height-40, _window_width-30, 1) andColor:colorf0 andView:shopView];
    ...
    UIView *SWPayView = [[UIView alloc]initWithFrame:CGRectMake(0, shopView.bottom + 10, _window_width, 140)];
    SWPayView.backgroundColor = [UIColor whiteColor];
    [_backScrollView addSubview:SWPayView];
    UILabel *leftL = [[UILabel alloc]init];
    leftL.textColor = color32;
    leftL.numberOfLines = 0;
    leftL.font = SYS_Font(13);
    leftL.text = @"订单编号:\n\n下单时间:\n\n支付状态:\n\n支付方式:";
    [SWPayView addSubview:leftL];
    [leftL mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(SWPayView).offset(15);
        make.centerY.equalTo(SWPayView);
    }];
    ...
    [SWPayView addSubview:rightL];
    [rightL mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(SWPayView).offset(-15);
        make.height.equalTo(leftL).offset(8);
        make.left.equalTo(leftL.mas_right).offset(10);
        make.centerY.equalTo(SWPayView).offset(-1);
    }];
    CGFloat nextBottom = SWPayView.bottom;
    if ([minstr([_orderMessage valueForKey:@"paid"]) isEqual:@"1"] && [minstr([_orderMessage valueForKey:@"status"]) intValue] > 0) {
        UIView *wuliuView = [[UIView alloc]initWithFrame:CGRectMake(0, SWPayView.bottom + 10, _window_width, 110)];
    ...
    [[SWToolClass sharedInstance] lineViewWithFrame:CGRectMake(15, 79, moneyView.width-30, 1) andColor:colorf0 andView:moneyView];
...
- (void)doConnectKefu{
    ...
    if ([minstr([_orderMessage valueForKey:@"mer_id"]) isEqual:[SWConfig getOwnID]]) {
        [MBProgressHUD showError:@"不能与自己发送私信"];
        return;
    }
    TUIConversationCellData *data = [[TUIConversationCellData alloc] init];
    data.userID = minstr([_orderMessage valueForKey:@"mer_id"]);    // 如果是单聊会话，传入对方用户 ID
    TUIChatController *chat = [[TUIChatController alloc] init];
    data.title = minstr([_orderMessage valueForKey:@"shop_name"]);    // 如果是单聊会话，传入对方用户 ID
...
- (void)doPingjiaGoosd:(UIButton *)sender{
    SWEvaluateWriteVC *vc = [[SWEvaluateWriteVC alloc]init];
    SWCartModel *model = self.goodsArray[sender.tag - 1000];
    vc.goodsModel = model;
    WeakSelf;
    vc.block = ^{
        model.is_reply = @"1";
...
- (void)checkAllGoodsReplay{
    ...
    for (SWCartModel *model in self.goodsArray) {
        if ([model.is_reply isEqual:@"0"]) {
            isAllReplay = NO;
        }
    }
...
- (void)doPayWithType:(NSString *)type{
    ...
    [SWToolClass postNetworkWithUrl:@"order/pay" andParameter:@{@"uni":minstr([_orderMessage valueForKey:@"order_id"]),@"paytype":type,@"from":@"ios"} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
...
- (void)doQueRen{
    ...
    [SWToolClass postNetworkWithUrl:@"order/take" andParameter:@{@"uni":minstr([_orderMessage valueForKey:@"order_id"])} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        [MBProgressHUD hideHUD];
        if (code == 200) {
...
- (void)showSenfGiftViewWithGain_integral:(NSInteger)gain_integral andGain_coupon:(NSInteger)gain_coupon{
    self.sendGiftView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, _window_width, _window_height)];
    self.sendGiftView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.3];
    [self.view addSubview:self.sendGiftView];
    UIView *whiteView = [[UIView alloc]init];
    whiteView.backgroundColor = [UIColor whiteColor];
    whiteView.layer.cornerRadius = 10;
    [self.sendGiftView addSubview:whiteView];
    [whiteView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.center.equalTo(self.sendGiftView);
        make.width.equalTo(self.sendGiftView).multipliedBy(0.75);
    }];
...
- (void)doDeateOrder{
    [MBProgressHUD showMessage:@""];
    [SWToolClass postNetworkWithUrl:@"order/del" andParameter:@{@"uni":minstr([_orderMessage valueForKey:@"order_id"])} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        [MBProgressHUD hideHUD];
...
- (void)doAgainBuyOrder{
    [MBProgressHUD showMessage:@""];
    [SWToolClass postNetworkWithUrl:@"order/again" andParameter:@{@"uni":minstr([_orderMessage valueForKey:@"order_id"])} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 200) {
...
- (void)getNeworderMessage{
    [SWToolClass getQCloudWithUrl:[NSString stringWithFormat:@"order/detail/%@?status=0",minstr([_orderMessage valueForKey:@"order_id"])] Suc:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        [MBProgressHUD hideHUD];
        if (code == 200) {
            _orderMessage = info;
            self.goodsArray = [NSMutableArray array];
