...
- (void)creatUI{
    ...
    for (int i = 0; i < array.count; i ++) {
        ...
        if (i == 0) {
            ...
            wuliuStar = vvvv;
            wuliuLabel = showLabel;
        }
    }
    wordAndPicView = [[UIView alloc]initWithFrame:CGRectMake(15, view.bottom + 150, _window_width-30, 180)];
...
- (void)didClickStarView:(WYStarView *)starView andCurrentScore:(CGFloat)score{
    ...
    }else if (starView == fuwuStar) {
        fuwuLabel.text = [NSString stringWithFormat:@"%.0f分",score];
    }else{
        wuliuLabel.text = [NSString stringWithFormat:@"%.0f分",score];
    }
}
...
- (void)doPingjia:(NSArray *)array{
    ...
    NSDictionary *dic = @{
        ...
        @"express_score":[NSString stringWithFormat:@"%.0f",wuliuStar.currentScore],//fuwuStar.currentScore == 0 ? @"5" :
