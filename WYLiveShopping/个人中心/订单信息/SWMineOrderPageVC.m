...
#import "SWMineOrderPageVC.h"
#import "TYPagerController.h"
#import "TYTabPagerBar.h"
#import "SWOrderListVC.h"

@interface SWMineOrderPageVC ()<TYTabPagerBarDataSource,TYTabPagerBarDelegate,TYPagerControllerDataSource,TYPagerControllerDelegate>
...
@implementation SWMineOrderPageVC
...
- (void)creatHeaderView {
    ...
    [colorView addSubview:headerTitleLabel];
    ...
    self.numsLabel = [[UILabel alloc] init];
    self.numsLabel.font = SYS_Font(11);
    self.numsLabel.textColor = [[UIColor whiteColor] colorWithAlphaComponent:0.8];
    [colorView addSubview:self.numsLabel];
    [self.numsLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(headerTitleLabel);
        make.top.equalTo(headerTitleLabel.mas_bottom).offset(14);
    }];
...
- (void)viewWillLayoutSubviews {
    ...
    self.tabBar.frame = CGRectMake(15, 83, _window_width - 30, 70);
    self.pagerController.view.frame = CGRectMake(0, self.naviView.bottom + 153, _window_width, _window_height - (self.naviView.bottom + 153) - ShowDiff);
}
...
- (void)viewDidLoad {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [self.pagerController scrollToControllerAtIndex:self.showIndex animate:YES];
        [self requestCount];
    });
}
...
- (void)loadData {
    self.datas = @[
        [NSString stringWithFormat:@"待付款\n\n%@", minstr([self.orderStatusNum valueForKey:@"unpaid_count"])],
        [NSString stringWithFormat:@"待发货\n\n%@", minstr([self.orderStatusNum valueForKey:@"unshipped_count"])],
        [NSString stringWithFormat:@"待收货\n\n%@", minstr([self.orderStatusNum valueForKey:@"received_count"])],
        [NSString stringWithFormat:@"待评价\n\n%@", minstr([self.orderStatusNum valueForKey:@"evaluated_count"])],
        [NSString stringWithFormat:@"已完成\n\n%@", minstr([self.orderStatusNum valueForKey:@"complete_count"])],
    ].mutableCopy;
    [self reloadData];
}
...
- (UIViewController *)pagerController:(TYPagerController *)pagerController controllerForIndex:(NSInteger)index prefetching:(BOOL)prefetching {
    WeakSelf;
    SWOrderListVC *vc = [[SWOrderListVC alloc] init];
...
- (void)requestCount {
    [SWToolClass getQCloudWithUrl:@"order/data?status=0" Suc:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 200) {
            self.numsLabel.text = [NSString stringWithFormat:@"消费订单：%@    总消费：¥ %@", minstr([info valueForKey:@"order_count"]), minstr([info valueForKey:@"sum_price"])];
            self.datas = @[
