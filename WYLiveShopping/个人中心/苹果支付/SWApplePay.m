...
#import "SWConfig.h"
@interface SWApplePay ()<SKProductsRequestDelegate,SKPaymentTransactionObserver>
...
-(void)SWApplePay:(NSDictionary *)dic{
    ...
    [self.delegate SWApplePayShowHUD];
    NSDictionary *dics = @{
    ...
    [SWToolClass postNetworkWithUrl:@"Charge.getIosOrder" andParameter:dics success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 0) {
    ...
    else
    {
        ...
        if ([[dict valueForKey:@"status"] isEqual:@"success"]) {
          //比对字典中以下信息基本上可以保证数据安全
            ...
            [self.delegate SWApplePaySuccess];
        }
        else
        {
...
- (void)showAlertView:(NSString *)message
{
    ...
    [[SWMXBADelegate sharedAppDelegate].topViewController presentViewController:alertContro animated:YES completion:nil];
}
@end
