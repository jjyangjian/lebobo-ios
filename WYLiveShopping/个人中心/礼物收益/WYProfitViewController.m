...
- (void)requestData{
    [WYToolClass getQCloudWithUrl:@"votes" Suc:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        ...
        cash_rate = [minstr([info valueForKey:@"votes_per"]) floatValue];
...
- (void)ChangeMoenyLabelValue{
    ...
    moneyLabel.text = [NSString stringWithFormat:@"¥%d",(int)([votesT.text integerValue]/cash_rate)];
    if ([[NSString stringWithFormat:@"%d",(int)([votesT.text integerValue]/cash_rate)] integerValue] > 0) {
