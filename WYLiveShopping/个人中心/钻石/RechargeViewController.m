...
//    payTypeID = minstr([payTypeSelDic valueForKey:@"id"]);
    payTypeID = minstr([self.payArr[sender.tag - 1000] valueForKey:@"id"]);
    ...
    for (int i = 0; i < coinArray.count; i++) {
        ...
        if ([payTypeID isEqual:@"apple"]) {
            label.text = minstr([allArray[i] valueForKey:@"coin_ios"]);
        }else{
            label.text = minstr([allArray[i] valueForKey:@"coin"]);
        }
    }
}
