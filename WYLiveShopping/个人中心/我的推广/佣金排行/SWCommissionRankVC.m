...
@interface SWCommissionRankVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation SWCommissionRankVC
...
//        [btn setBackgroundImage:[SWToolClass getImgWithColor:normalColors] forState:0];
    ...
    self.moveLineView = [[UIView alloc]initWithFrame:CGRectMake(viewWidth/4-25, 39, 50, 1)];
    self.moveLineView.backgroundColor = normalColors;
    [view addSubview:self.moveLineView];

    
    self.listTableView = [[UITableView alloc]initWithFrame:CGRectZero style:0];
...
- (void)requestData{
    [SWToolClass getQCloudWithUrl:[NSString stringWithFormat:@"brokerage_rank?page=%ld&type=%@&limit=20",(long)self.page,self.typeString] Suc:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
            ...
            if (code == 200) {
                if (self.page == 1) {
                    ...
                    if ([minstr([info valueForKey:@"position"]) intValue] == 0) {
                        self.rankLabel.text = @"您目前暂无排名";
                    }else{
                        self.rankLabel.attributedText = [self setAttText:minstr([info valueForKey:@"position"])];
                    }
                }
                NSArray *list = [info valueForKey:@"rank"];
                for (NSDictionary *dic in list) {
                    SWCommissionUserModel *model = [[SWCommissionUserModel alloc]initWithDic:dic];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.comModel = self.dataArray[indexPath.row];
