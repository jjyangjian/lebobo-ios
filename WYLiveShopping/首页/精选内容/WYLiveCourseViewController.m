...
@interface WYLiveCourseViewController ()<TYTabPagerBarDataSource,TYTabPagerBarDelegate,TYPagerControllerDataSource,TYPagerControllerDelegate>{
...
@implementation WYLiveCourseViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.titleL.text = @"精选内容";
...
- (UIViewController *)pagerController:(TYPagerController *)pagerController controllerForIndex:(NSInteger)index prefetching:(BOOL)prefetching {
    ...
    if ([vue isKindOfClass:[NSString class]]) {
        courseClassViewController *VC = [[courseClassViewController alloc]init];
        VC.type = minstr([_datas[index] valueForKey:@"id"]);
...
- (void)requestData{
    [WYToolClass getQCloudWithUrl:@"courseindex" Suc:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 200) {
            NSArray *cat = [info valueForKey:@"cat"];
            _datas = [cat mutableCopy];
