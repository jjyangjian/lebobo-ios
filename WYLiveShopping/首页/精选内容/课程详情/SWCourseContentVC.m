...
@interface SWCourseContentVC ()<WMPlayerDelegate,SWAudioPlayViewDelegate>
@property (nonatomic,strong) UIView *headerView;
@property (nonatomic,strong) UIScrollView *bottomScrollView;
@property (nonatomic,strong) SWAudioPlayView *voiceView;
@property (nonatomic,strong) UIButton *tryBuyButton;
...
@implementation SWCourseContentVC
- (void)creatHeaderView{
    self.headerView = [[UIView alloc]initWithFrame:CGRectMake(0, 64+statusbarHeight, _window_width, _window_width*0.56)];
...
//    }
    if ([_model.type isEqual:@"1"] || [_model.type isEqual:@"3"]) {
        ...
        if ([_model.type isEqual:@"3"]) {
            UIImageView *imgV = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, _window_width, self.headerView.height)];
            ...
            self.headerView.height = _window_width*0.56 +125;
            self.voiceView = [[[NSBundle mainBundle] loadNibNamed:@"SWAudioPlayView" owner:nil options:nil] lastObject];
            self.voiceView.frame = CGRectMake(0, imgV.bottom, _window_width, 120);
            self.voiceView.delegate = self;
            self.voiceView.courseDic = _courseMsgDic;
            [self.headerView addSubview:self.voiceView];
            [[SWToolClass sharedInstance]lineViewWithFrame:CGRectMake(0, self.voiceView.bottom, _window_width, 5) andColor:colorf0 andView:self.headerView];
        }else{
...
- (void)creatBottomView{
    self.bottomScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, self.headerView.bottom, _window_width, _window_height-ShowDiff-(self.headerView.bottom))];
    ...
    [self.bottomScrollView addSubview:titleL];
    [titleL mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.bottomScrollView).offset(15);
        make.top.equalTo(self.bottomScrollView).offset(15);
        make.width.equalTo(self.bottomScrollView).offset(-30);
    }];
    self.jianjieLabel = [[UILabel alloc]init];
    [self.bottomScrollView addSubview:self.jianjieLabel];
    self.jianjieLabel.textColor = color96;
    self.jianjieLabel.font = SYS_Font(11);
    ...
    self.timeLabel.textColor = color96;
    self.timeLabel.font = SYS_Font(11);
//    timeLabel.text = _model.time_date;
    [self.timeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.jianjieLabel.mas_bottom).offset(20);
        make.left.equalTo(titleL);
    }];
    UIView *lineV = [[UIView alloc]init];
    lineV.backgroundColor = colorf0;
    [self.bottomScrollView addSubview:lineV];
    ...
    [self.bottomScrollView addSubview:self.webView];
    [self.webView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(lineV.mas_bottom).offset(20);
        make.left.width.equalTo(self.bottomScrollView);
        make.height.equalTo(self.bottomScrollView);
    }];
...
//        timeLabel.text = _model.time_date;
//
//        request = [NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/appapi/course/lesson?uid=%@&token=%@&lessonid=%@",h5url,[SWConfig getOwnID],[SWConfig getOwnToken],_model.lessonID]]];
//
//    }else{
...
//        request = [NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/appapi/course/content?uid=%@&token=%@&courseid=%@",h5url,[SWConfig getOwnID],[SWConfig getOwnToken],minstr([_courseMsgDic valueForKey:@"id"])]]];
//
//    }
...
//    }];
    ...
    self.bottomScrollView.contentSize = CGSizeMake(0, self.webView.bottom + ShowDiff + 20);
}
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)requestCourseDetaile{
    ...
    [SWToolClass getQCloudWithUrl:[NSString stringWithFormat:@"lessondetail&lessonid=%@",_model.lessonID] Suc:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
...
//                    model.videoURL = [NSURL URLWithString:@"http://clips.vorwaerts-gmbh.de/big_buck_bunny.mp4"];
                ...
                if ([_model.type intValue] == 3) {
                    [self.voiceView playerReadyToPlay:minstr([info valueForKey:@"url"])];
                }
                self.jianjieLabel.text = minstr([info valueForKey:@"des"]);
                self.timeLabel.text = minstr([info valueForKey:@"add_time"]);
...
#pragma mark -- SWAudioPlayViewDelegate
