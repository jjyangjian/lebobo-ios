...
@interface WYAllVideoViewController ()<TYTabPagerBarDataSource,TYTabPagerBarDelegate,TYPagerControllerDataSource,TYPagerControllerDelegate>{
...
@implementation WYAllVideoViewController
...
- (void)requestData{
    [WYToolClass getQCloudWithUrl:@"videocat&type=1" Suc:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 200) {
            _datas = [info mutableCopy];
...
- (UIViewController *)pagerController:(TYPagerController *)pagerController controllerForIndex:(NSInteger)index prefetching:(BOOL)prefetching {
    ...
    if ([vue isKindOfClass:[NSString class]]) {
        WYClassVideoListViewController *VC = [[WYClassVideoListViewController alloc]init];
