...
//    WHUCalDrawView* _SmallcalDrawView;
...
- (void)requestCourseState{
    ...
    [WYToolClass postNetworkWithUrl:@"Schedule.GetMark" andParameter:@{@"time":selectDateString} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        [MBProgressHUD hideHUD];
        if (code == 0) {
            _youkeArray = info;
            dispatch_async(dispatch_get_main_queue(), ^{
                NSArray *aaarrr = _dataDic[@"dataArr"];
                for (WHUCalendarItem *item in aaarrr) {
...
//    for (WHUCalendarItem *item in arrr) {
//        if ([_youkeArray containsObject:[NSString stringWithFormat:@"%ld",item.day]]) {
//            item.isHasCourse = YES;
//        }
//    }
...
//        });
//    };
//    _calDrawView.blockkkkkkkkk = ^(WHUCalendarItem *item) {
//        dispatch_async(dispatch_get_main_queue(), ^{
...
- (WHUCalendarItem *)getIteamWithDate:(NSDate *)date{
