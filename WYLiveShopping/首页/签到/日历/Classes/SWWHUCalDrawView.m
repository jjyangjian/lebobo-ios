...
@property (nonatomic,strong) NSMutableArray *greenViewArray;;

@end
@implementation SWWHUCalDrawView
+ (Class)layerClass
{
        ...
        _tagDic=nil;
            ...
            dispatch_async(_calcal.workQueue,^{
                ...
                for(NSInteger i=0;i<tempArr.count;i++){
                    SWWHUCalendarItem* item=tempArr[i];
...
- (void)drawRect:(CGRect)rect {
    ...
    if(selectIndex!=NSNotFound && tempArr.count >= _rectArray.count){
        CGRect r = [[_rectArray objectAtIndex:selectIndex] CGRectValue];
        [self fillContext:context color:normalColors rect:r];
    }
...
//        }
        ...
        if(item.day<0){
            ...
            if (item == _selectedDateItem) {
                [dayStr addAttribute:(id)kCTForegroundColorAttributeName value:[UIColor whiteColor] range:NSMakeRange(0, dayStr.length)];
            }else{
                [dayStr addAttribute:(id)kCTForegroundColorAttributeName value:[UIColor lightGrayColor] range:NSMakeRange(0, dayStr.length)];
            }
//            UIView *vvvvv = _greenViewArray[[_rectArray indexOfObject:v]];
//            vvvvv.hidden = YES;
//                [holiStr addAttribute:(id)kCTForegroundColorAttributeName value:[UIColor lightGrayColor] range:NSMakeRange(0, holiStr.length)];
            ...
            if([item.dateStr isEqualToString:[_calcal currentDateStr]]){
                dayStr=[[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%ld\n%@",(long)item.day,_isSignIn ? @"已签" : @"今天"]];
                [dayStr addAttribute:(id)kCTForegroundColorAttributeName value:normalColors range:NSMakeRange(0, dayStr.length)];

//                if ([item.dateStr isEqual:_selectedDateItem.dateStr] || !_selectedDateItem) {
//                    [dayStr addAttribute:(id)kCTForegroundColorAttributeName value:[UIColor whiteColor] range:NSMakeRange(0, dayStr.length)];
//                    NSInteger selectIndex=[tempArr indexOfObject:item];
...
//                    if(selectIndex!=NSNotFound){
//                        CGRect r = [[_rectArray objectAtIndex:selectIndex] CGRectValue];
//                        [self fillContext:context color:nil rect:r];
//                    }
//
//                }
//                [holiStr addAttribute:(id)kCTForegroundColorAttributeName value:[UIColor blueColor] range:NSMakeRange(0, holiStr.length)];
                ...
                dayStr=[[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%ld",(long)item.day]];
                [dayStr addAttribute:(id)kCTForegroundColorAttributeName value:color32 range:NSMakeRange(0, dayStr.length)];

//                [holiStr addAttribute:(id)kCTForegroundColorAttributeName value:[UIColor blackColor] range:NSMakeRange(0, holiStr.length)];
//                if ([item.dateStr isEqual:_selectedDateItem.dateStr]) {
//                    [dayStr addAttribute:(id)kCTForegroundColorAttributeName value:[UIColor whiteColor] range:NSMakeRange(0, dayStr.length)];
//                }else{
//                    [dayStr addAttribute:(id)kCTForegroundColorAttributeName value:[UIColor blackColor] range:NSMakeRange(0, dayStr.length)];
//                    NSInteger curIndex =[tempArr indexOfObject:item];
//                    NSLog(@"curIndex = %d",item.day);
//
//                    if(curIndex!=NSNotFound && item.isHasCourse){
//                        CGRect r = [[_rectArray objectAtIndex:curIndex] CGRectValue];
//                        [self addYuanDIContext:context color:normalColors rect:r];
//                    }
//
//                }
        ...
        [dayStr addAttribute:NSFontAttributeName value:[UIFont boldSystemFontOfSize:12]range:NSMakeRange(0, dayStr.length)];
...
- (void)addYuanDIContext:(CGContextRef)context color:(UIColor*)color rect:(CGRect)r{
    [[UIColor whiteColor] set];
    CGContextDrawPath(context, kCGPathFillStroke);
    CGContextFillRect(context, r);
    if (color) {
        CGContextSetFillColorWithColor(context, [normalColors colorWithAlphaComponent:0.1].CGColor);
        CGContextAddArc(context,r.origin.x+r.size.width/2,r.origin.y+r.size.width/2,r.size.width/2-5,0,2*M_PI,0);//添加一个圆
    }
    CGContextDrawPath(context, kCGPathFillStroke);
...
//    CGPathAddRect(path, NULL, r);
    ...
    CGContextDrawPath(context, kCGPathFillStroke);
    CGContextFillRect(context, r);
    CGFloat dashArray[] = {0, 0};
    if (color){

        CGContextSetLineWidth(context, 5.0);
        //画虚线
        CGContextSetLineDash(context, 1, dashArray, 1);
        CGContextSetStrokeColorWithColor(context, RGB_COLOR(@"#eaf2ff", 1).CGColor);
        CGContextSetFillColorWithColor(context, normalColors.CGColor);
        CGContextAddArc(context,r.origin.x+r.size.width/2,r.origin.y+r.size.width/2,r.size.width/2-5,0,2*M_PI,0);//添加一个圆
    }else{
        CGContextSetLineWidth(context, 1.0);
        //画虚线
        CGContextSetLineDash(context, 1, dashArray, 1);
        CGContextAddArc(context,r.origin.x+r.size.width/2,r.origin.y+r.size.width/2,r.size.width/2-10,0,2*M_PI,0);//添加一个圆
        CGContextSetStrokeColorWithColor(context, normalColors.CGColor);
        CGContextDrawPath(context, kCGPathFillStroke);
    }
...
//
//    CGContextAddArc(context,r.origin.x+r.size.width/2,r.origin.y+r.size.width/2,r.size.width/2,0,2*M_PI,0);//添加一个圆
//    CGContextSetFillColorWithColor(context, normalColors.CGColor);
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if(canSelect&&_selectedDateItem!=nil){
        ...
        dispatch_after(time, dispatch_get_main_queue(), ^{
            ...
            if (self.blockkkkkkkkk) {
                self.blockkkkkkkkk(_selectedDateItem);
            }
        });
    }
    [self setNeedsDisplay];
...
//    _greenViewArray = [NSMutableArray array];
//    for (int i = 0; i < _rectArray.count; i ++) {
//        NSValue* v = _rectArray[i];
//        CGRect cccccc = [v CGRectValue];
//        UIView *vvvvvv = [[UIView alloc]initWithFrame:CGRectMake(cccccc.size.width/2 - 3 + (i%7)*cccccc.size.width, cccccc.size.height - 6 + (i/7)*cccccc.size.height, 6, 6)];
//        vvvvvv.layer.cornerRadius = 3;
//        vvvvvv.backgroundColor = normalColors;
////        vvvvvv.hidden = YES;
//        [self addSubview:vvvvvv];
//        [_greenViewArray addObject:vvvvvv];
//    }
    return  path;
}
@end
