...
@implementation SWWHUCalendarCal
...
-(NSDictionary*)loadDataWith:(NSString*)dateStr andIsSmall:(BOOL)isSmall{
    if (isSmall) {
        self.currentCalMap = [self latelyEightTime:dateStr];
    }else{
...
-(NSDictionary *)latelyEightTime:(NSString *)mouthStr{
    NSMutableDictionary *muDic = [NSMutableDictionary dictionary];
    [muDic setObject:mouthStr forKey:@"monthStr"];
    ...
    for (int i = 0; i < 7; i ++) {
        //从现在开始的24小时
        NSTimeInterval secondsPerDay = i * 24*60*60-1;
        ...
        NSString *dateStr2 = [dateFormatter2 stringFromDate:curDate];//几月几号

        SWWHUCalendarItem* item=[[SWWHUCalendarItem alloc] init];
        item.dateStr = dateStr2;
        item.day = [dateStr integerValue];
        [weekArr addObject:item];
    }
    [muDic setObject:[weekArr copy] forKey:@"dataArr"];
    return muDic;
}
...
-(void)LunarForSolarYear:(SWWHUCalendarItem *)calendarDay andComponent:(NSDateComponents*)com{
