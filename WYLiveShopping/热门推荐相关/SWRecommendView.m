...
- (UICollectionView *)recommendCollectionView {
    if (!_recommendCollectionView) {
        ...
        [_recommendCollectionView registerNib:[UINib nibWithNibName:@"SWHotGoodsCell" bundle:nil] forCellWithReuseIdentifier:@"hotGoodsCELL"];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    SWHotGoodsCell *cell = (SWHotGoodsCell *)[collectionView dequeueReusableCellWithReuseIdentifier:@"hotGoodsCELL" forIndexPath:indexPath];
...
- (UIView *)headerView {
    if (!_headerView) {
        ...
        if (self.nothingImage) {
            ...
            UIImageView *nothingImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 20, _window_width, _window_width * 0.55)];
            nothingImageView.image = self.nothingImage;
            nothingImageView.contentMode = UIViewContentModeScaleAspectFit;
            [_headerView addSubview:nothingImageView];
            viewTop = nothingImageView.bottom + 16;
        } else {
...
- (void)requestData {
    [SWToolClass getQCloudWithUrl:[NSString stringWithFormat:@"product/hot?page=%d&limit=20", self.page] Suc:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
