...
- (void)onEnterRoom:(NSInteger)result {
    ...
    NSLog(@"[TRTC] onEnterRoom result=%ld roomId=%@ userId=%@",
          (long)result, minstr([_roomDic objectForKey:@"showid"]), [Config getOwnID]);
    _trtcEntering = NO;
...
- (void)creatRommStepSecond:(NSString *)thumbUrl{
    ...
    [WYToolClass postNetworkWithUrl:@"live/start" andParameter:dic success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        ...
        [LinkmicManager shareInstance].linkSwitch = NO;
        [LinkmicManager shareInstance].isLinking = NO;
        [self startUI];
    } fail:^{
...
-(void)clickLinkBtn {
    BOOL micSwitch = [LinkmicManager shareInstance].linkSwitch;
    if (micSwitch == NO) {
        [self showLinkAlert];
    }else {
        [self showApplyView];
...
-(void)showApplyView {
    BOOL isLinking = [LinkmicManager shareInstance].isLinking;
    if(_linkUserView){
    ...
    if (!applyView) {
        applyView = [[LinkApplyView alloc]initWithFrame:CGRectMake(0, 0, _window_width, _window_height) andLiveUid:[Config getOwnID]];
    ...
    // 已读
    [[LinkmicManager shareInstance] resetNums];
    [self judgeRedShow];
}
-(void)showLinkAlert {
    ...
    UIAlertAction *sureAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        // 开启
        [[LinkmicManager shareInstance] livemicSwitch:1];
        // socket
...
-(void)getCloseShow
{
    ...
    [WYToolClass postNetworkWithUrl:@"live/stop" andParameter:@{@"stream":minstr([_roomDic valueForKey:@"stream"])} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        ...
        // 服务端在关播处处理
        [[LinkmicManager shareInstance] livemicStop];
    } fail:^{
        [_socketL closeRoom];//发送关闭直播的socket
        [_socketL colseSocket];//注销socket
        _socketL = nil;//注销socket
...
#warning rk_是否需要？
    // 关播-关闭连麦开关
    if ([LinkmicManager shareInstance].linkSwitch) {
        [[LinkmicManager shareInstance] livemicSwitch:0];
    }
...
- (void)showgoodsShowView{
    ...
    if (!goodsView) {
        goodsView = [[liveGoodsView alloc]initWithFrame:CGRectMake(0, 0, _window_width, _window_height) andLiveUid:[Config getOwnID]];
...
- (void)agreeUserLink:(NSDictionary *)userInfo {
    ...
    [[LinkmicManager shareInstance] livemicManage:YES andUserid:userid handle:^(LinkEvent event, int eventCode, NSDictionary *eventDic) {
        if (eventCode == 0) {
            showEndLinkBtn = true;
            dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [LinkmicManager shareInstance].isLinking = YES;
            BOOL isLinking = [LinkmicManager shareInstance].isLinking;
            NSLog(@"-----------------%ld",[LinkmicManager shareInstance].isLinking);
        }
    }];
}
...
-(void)createTRTCLinkUserViewWithUserId:(NSString *)userId {
    [self destroyLinkUserView];
    _linkUserView = [[LinkUserView alloc] initWithTRTCRemoteUserId:userId
...
- (void)linkmicEventSoc:(NSDictionary *)linkDic {
    ...
    if (action == 1) {
        // 显示红点+1
        [[LinkmicManager shareInstance] applyNumsChange:YES];
        [self judgeRedShow];
    }else if(action == 2) {
        // 显示红点-1
        [[LinkmicManager shareInstance] applyNumsChange:NO];
        [self judgeRedShow];
    }else if (action == 5){
        ...
        [self trtcEnterRoomIfNeededThen:^(BOOL ok) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                LinkUserView *linkView = strongSelf->_linkUserView;
...
-(void)judgeRedShow {
    if ([LinkmicManager shareInstance].currentApplayNums<=0) {
        linkRed.hidden = YES;
    }else {
        linkRed.hidden = NO;
    }
