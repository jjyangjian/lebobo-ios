...
@interface SWLivePlayerViewController ()<TRTCCloudDelegate,UITextViewDelegate,socketLiveDelegate,UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,haohuadelegate,TXLivePlayListener,anchorMessageViewDelegate,WYGiftListViewDelegate,LiveOnlineListDelegate,SWUserPopupViewDeleagte,tx_play_linkmic,V2TXLivePlayerObserver>{
    ...
    BOOL isShutUp;
    //主播信息弹窗
    SWAnchorMessageView *anchorMsgV;
    SWLiveOnlineList *onlineList;//在线用户列表
    ///第一次点亮
    BOOL isLight;
    ///用户更多操作
    SWUserMoreView *moreView;
    
    CGSize videoSize;
    SWGiftListView *giftview;//礼物界面

    SWLinkmicItem *linkItem;
...
@implementation SWLivePlayerViewController
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //&& ![model.userID isEqual:[SWConfig getOwnID]]
...
- (void)doShowAnchorView{
    if (!anchorMsgV) {
        ...
        anchorMsgV = [[[NSBundle mainBundle] loadNibNamed:@"SWAnchorMessageView" owner:nil options:nil] lastObject];
...
- (void)addLeftView{
    ...
    // 连麦按钮
    linkItem = [[SWLinkmicItem alloc]init];
...
- (void)showmoreview{
    if (!moreView) {
        moreView = [[SWUserMoreView alloc]initWithFrame:CGRectMake(moreBtn.centerX-30, moreBtn.top-40, 60, 34)];
        WeakSelf;
...
- (void)doShowGiftView{
    if (!giftview) {
        ...
        [giftview chongzhiV:[SWConfig getNowIcon]];
    }
...
//    SWLiveUser *users = [SWConfig myProfile];
//    
//    [SWConfig updateProfile:users];
    [_socketL sendGiftAndINfo:info andlianfa:lianfa];
}
- (void)pushCoinV{
    SWRechargeVC *rechargeVC = [SWRechargeVC new];
