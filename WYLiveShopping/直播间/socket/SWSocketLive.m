...
-(void)addNodeListen:(NSString *)socketUrl andRoomMessage:(NSDictionary *)roomDic{
    ...
    __weak SWSocketLive *weakself = self;
...
// 连麦
-(void)linkMicSocket:(int)action andParam:(NSDictionary *)paramDic {
    ...
    NSMutableDictionary *m_msg_dic = [NSMutableDictionary dictionaryWithDictionary:baseDic];

    if (action == 1 || action == 2 || action == 6 || action == 8 || action == 9) {

        [m_msg_dic setObject:[SWConfig getOwnID] forKey:@"uid"];
        [m_msg_dic setObject:[SWConfig getOwnNicename] forKey:@"name"];

    }else if (action == 3 || action == 4 || action == 7) {

        [m_msg_dic setObject:minstr([paramDic valueForKey:@"to_uid"]) forKey:@"to_uid"];

    }else if (action == 5) {

        [m_msg_dic setObject:[SWConfig getOwnID] forKey:@"uid"];
        [m_msg_dic setObject:minstr([paramDic valueForKey:@"user_pull"]) forKey:@"user_pull"];

    }
    NSDictionary *sendDic = [NSDictionary dictionaryWithDictionary:m_msg_dic];
    NSArray *msgData =@[
        @{
            @"msg": @[
                sendDic
