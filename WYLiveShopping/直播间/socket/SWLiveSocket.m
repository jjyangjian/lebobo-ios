                        ...
                        @{
                            @"msg": @[
                                    @{
                                        ...
                                        @"usernickname": [SWConfig getOwnNicename],
                                        @"avatar":[SWConfig getavatar],
                                        @"isattent": isAtt,
                         ...
                         @{
                             @"msg": @[
                                     @{
                                         ...
                                         @"uid": [SWConfig getOwnID],
                                         @"uname": [SWConfig getOwnNicename],
                                         @"equipment": @"app",
                                         @"roomnum": [SWConfig getOwnID]
                                         }
                                     ],
                             @"retcode": @"000000",
                             @"retmsg": @"OK"
...
-(void)addNodeListen:(NSString *)socketUrl andRoomMessage:(NSDictionary *)roomMap{
    _roomMap = roomMap;
    roomStream = minstr([_roomMap valueForKey:@"stream"]);
    __weak SWLiveSocket *weakself = self;
    ...
    NSArray *cur = @[@{@"username":[SWConfig getOwnNicename],
                       @"uid":[SWConfig getOwnID],
                       @"token":minstr([_roomMap valueForKey:@"token"]),
                       @"roomnum":minstr([_roomMap valueForKey:@"uid"]),
                       @"stream":roomStream
                       }];
    [ChatSocket connect];
...
-(void)getmessage:(NSDictionary *)msg andMethod:(NSString *)method{
    ...
    if ([method isEqual:@"SendMsg"]) {
        ...
        if (action == 1) {
            [self.delegate receiveMessage:msg];
        }
    }
        ...
        if ([[msg valueForKey:@"ct"] isKindOfClass:[NSDictionary class]]) {
            ...
            ct = [[SWToolClass sharedInstance] dictionaryWithJsonString:[msg valueForKey:@"ct"]];
        }
    ...
    else if ([method isEqual:@"Linkmic"]) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(linkmicEventSoc:)]) {
            [self.delegate linkmicEventSoc:msg];
        }
    }
}
...
// 连麦
-(void)linkMicSocket:(int)action andParam:(NSDictionary *)paramMap {
    // action:  1 用户申请  2用户取消  3主播同意 4主播拒绝  5 用户上麦 6用户下麦  7 主播踢用户下麦  8开启  9关闭

    NSDictionary *baseMap = @{
        @"_method_": @"Linkmic",
        @"action": @(action),
    };
    NSMutableDictionary *messageMap = [NSMutableDictionary dictionaryWithDictionary:baseMap];

    if (action == 1 || action == 2 || action == 6 || action == 8 || action == 9) {

        [messageMap setObject:[SWConfig getOwnID] forKey:@"uid"];
        [messageMap setObject:[SWConfig getOwnNicename] forKey:@"name"];

    }else if (action == 3 || action == 4 || action == 7) {

        [messageMap setObject:minstr([paramMap valueForKey:@"to_uid"]) forKey:@"to_uid"];

    }else if (action == 5) {

        [messageMap setObject:[SWConfig getOwnID] forKey:@"uid"];
        [messageMap setObject:minstr([paramMap valueForKey:@"user_pull"]) forKey:@"user_pull"];

    }
    NSDictionary *sendMap = [NSDictionary dictionaryWithDictionary:messageMap];
    NSArray *msgData =@[
        @{
            @"msg": @[
                sendMap
                        ...
                        @{
                            @"msg": @[
                                    @{
                                        ...
                                        @"uname": [SWConfig getOwnNicename],
                                        @"equipment": @"app",
                                        @"roomnum": [self.roomMap valueForKey: @"uid"],
                                        
                                        @"evensend":lianfa,
                                        @"uhead":[SWConfig getavatar],
                                        
                                        
                                        @"livename":[self.roomMap valueForKey:@"user_nicename"]
                                        }
                                    ],
                            @"retcode": @"000000",
                            @"retmsg": @"OK"
