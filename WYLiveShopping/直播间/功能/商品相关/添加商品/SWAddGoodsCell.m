...
- (IBAction)addGoodsBtnClick:(id)sender {
    ...
    if (_model.isAdmin) {
        [SWToolClass postNetworkWithUrl:@"shopadd" andParameter:@{@"productid":_model.goodsID} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
            if (code == 200) {
        ...
        [SWToolClass postNetworkWithUrl:@"setsale" andParameter:@{@"productid":_model.goodsID,@"issale":issale} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
            if (code == 200) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [MBProgressHUD hideHUD];
                    if (self.delegate) {
