...
@implementation SWSuspensionPlayer
...
- (void)fanhuishitu{
    [self stopPlay];
    NSArray *temArray = [SWMXBADelegate sharedAppDelegate].topViewController.navigationController.viewControllers;
    ...
    if (playerVC) {
        [[SWMXBADelegate sharedAppDelegate] popToViewController:playerVC];
    }else{
        [self removeFromSuperview];
        [self checkLive];
    }
