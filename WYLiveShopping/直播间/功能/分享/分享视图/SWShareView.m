...
- (void)creatUI{
    ...
    for (int i = 0; i < array.count; i ++) {
        ...
        btn = [SWToolClass setUpImgDownText:btn space:10];
...
- (UIImage *)getImage:(UIView *)SWShareView

{
    
    UIGraphicsBeginImageContextWithOptions(CGSizeMake(SWShareView.frame.size.width,SWShareView.frame.size.height ), NO, 0.0); //currentView 当前的view  创建一个基于位图的图形上下文并指定大小为
    
    [SWShareView.layer renderInContext:UIGraphicsGetCurrentContext()];
