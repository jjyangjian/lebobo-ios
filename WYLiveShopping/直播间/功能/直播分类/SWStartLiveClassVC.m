...
#import "SWStartLiveClassCell.h"
@interface SWStartLiveClassVC ()<UITableViewDelegate,UITableViewDataSource>
@property (nonatomic, strong) UITableView *classTableView;
@property (nonatomic, strong) NSArray *classArray;

@end

@implementation SWStartLiveClassVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.automaticallyAdjustsScrollViewInsets = NO;
//    self.classArray = [SWCommon liveclass];
    self.titleL.text = @"直播分类";
    self.classTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 64+statusbarHeight, _window_width, _window_height-64-statusbarHeight) style:UITableViewStylePlain];
    self.classTableView.delegate = self;
    self.classTableView.dataSource = self;
    self.classTableView.separatorStyle = 0;
    if (@available(iOS 15.0, *)) {
        self.classTableView.sectionHeaderTopPadding = 0;
    } else {
...
- (void)requestData{
    [SWToolClass getQCloudWithUrl:@"live/class&type=0" Suc:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    SWStartLiveClassCell *cell = [tableView dequeueReusableCellWithIdentifier:@"startLiveClassCELL"];
    if (!cell) {
