...
@implementation SWGiftCell

- (void)awakeFromNib {
    [super awakeFromNib];
    // Initialization code
}
-(void)setModel:(SWLiwuModel *)model{
    _model = model;
//    self.giftIcon.yy_imageURL = [NSURL URLWithString:_model.imagePath];
    [self.giftIcon sd_setImageWithURL:[NSURL URLWithString:_model.imagePath]];
    _giftNameL.text = _model.giftname;
    _giftCoinL.text = [NSString stringWithFormat:@"%@%@",_model.price,[SWCommon name_coin]];
