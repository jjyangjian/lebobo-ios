...
#import "SWGiftListView.h"
#import "SWGiftCell.h"
#import "SWLiwuModel.h"
#import "SWPublicObj.h"
#import "SWLWLCollectionViewHorizontalLayout.h"
#define celll @"cell"
@interface SWCollectionCellWhite : UICollectionViewCell
@end

@implementation SWCollectionCellWhite
...
@end
@interface SWGiftListView ()<UICollectionViewDelegate,UICollectionViewDataSource>{
...
@property(nonatomic,strong)SWLiwuModel *selectModel;
...
#pragma mark - request
- (void)requestData{
    [SWToolClass getQCloudWithUrl:@"giftlist" Suc:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 200) {
    
            [SWConfig saveIcon:minstr([info valueForKey:@"coin"])];
...
//            SWLiveUser *liveUser = [SWConfig myProfile];
...
- (void)addAnimation:(NSIndexPath *)indexpath{
    for (SWGiftCell *cell in _collectionView.visibleCells) {
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.item >= self.models.count) {
        ...
        SWGiftCell *cell = (SWGiftCell *)[collectionView dequeueReusableCellWithReuseIdentifier:celll forIndexPath:indexPath];
        
        SWLiwuModel *model = self.models[indexPath.item];
...
- (void)creatBottomView{
    ...
    [haohuaBtn setBackgroundImage:[SWPublicObj getImgWithColor:normalColors] forState:UIControlStateSelected];
    [haohuaBtn setBackgroundImage:[SWPublicObj getImgWithColor:RGB_COLOR(@"#282518", 1)] forState:UIControlStateNormal];
    ...
    //SWLiveUser *user = [SWConfig myProfile];
...
- (void)creatAllGiftView{
    ...
    SWLWLCollectionViewHorizontalLayout *Flowlayout =[[SWLWLCollectionViewHorizontalLayout alloc]init];
    ...
    [self.collectionView registerNib:[UINib nibWithNibName:@"SWGiftCell" bundle:nil] forCellWithReuseIdentifier:celll];
    self.collectionView.dataSource = self;
    self.collectionView.delegate = self;
    self.collectionView.multipleTouchEnabled = NO;
    [self.collectionView registerClass:[SWCollectionCellWhite class]
