...
- (IBAction)doFollow:(id)sender {
    ...
    if (_followBtn.selected) {
        ...
        [SWToolClass postNetworkWithUrl:@"attent/del" andParameter:@{@"touid":anchorId} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        ...
        _followBtn.selected = !_followBtn.selected;
        [SWToolClass postNetworkWithUrl:@"attent/add" andParameter:@{@"touid":anchorId} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
            _followBtn.userInteractionEnabled = YES;
            if (code == 200) {
                [MBProgressHUD showError:msg];
                if (self.block) {
...
- (IBAction)doShop:(id)sender {
    SWStoreHomeTbabarViewController *vc = [[SWStoreHomeTbabarViewController alloc]initWithID:anchorId];
