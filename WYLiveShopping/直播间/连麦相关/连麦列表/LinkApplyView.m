...
@interface LinkApplyView()<UITableViewDelegate,UITableViewDataSource,LinkApplyDelegate>{
    int page;
    UIView *showView;
    UILabel *titleLable;
    NSString *liveUid;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    LinkApplyCell *cell = [tableView dequeueReusableCellWithIdentifier:@"LinkApplyCell"];
    if (!cell) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"LinkApplyCell" owner:nil options:nil] lastObject];
        cell.delegate = self;
    }
...
//        @"stream":minstr([[LinkmicManager shareInstance].roomDic valueForKey:@"stream"]),
//    };
    //
    NSString *getUrl = [NSString stringWithFormat:@"livemic/list&stream=%@&p=%@",minstr([[LinkmicManager shareInstance].roomDic valueForKey:@"stream"]),@(page)];
