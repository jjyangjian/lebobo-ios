...
@interface SWAppDelegate ()<WXApiDelegate, TXLiveBaseDelegate>
{
...
//    [[NSUserDefaults standardUserDefaults] setBool:NO forKey:@"isStartCall"];
    ...
    if ([SWConfig getOwnID] && [[SWConfig getOwnID] integerValue] > 0) {
        SWTabBarController *tabbar = [[SWTabBarController alloc] init];
        self.window.rootViewController = tabbar;

    }else{
        SWLoginViewController *tabbar = [[SWLoginViewController alloc] init];
...
- (void)onLicenceLoaded:(int)result Reason:(NSString *)reason {
    NSLog(@"TXLiveBase onLicenceLoaded: result:%d reason:%@", result, reason);
}
