...
@implementation SWLogInCodePutView{
    UIButton *logBtn;
    UIButton *codeBtn;
...
- (void)getCode{
    codeBtn.userInteractionEnabled = NO;
    [SWToolClass postNetworkWithUrl:@"register/verify" andParameter:@{@"phone":_nameT.text,@"type":@"login",@"key":codeKey} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
...
//    }
    ...
    [SWToolClass postNetworkWithUrl:@"login/mobile" andParameter:Login success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 200) {
