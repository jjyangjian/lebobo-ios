...
@implementation SWLoginPwdPutView{
    UITextField *nameT;
    UITextField *pwdT;
    UIButton *logBtn;
...
- (void)creatUI{
    ...
    for (int i = 0; i < array.count; i ++) {
        ...
        [[SWToolClass sharedInstance] lineViewWithFrame:CGRectMake(text.left, 69, text.width, 1) andColor:RGB_COLOR(@"#E8E8E8", 1) andView:view];
        if (i == 0) {
            nameT = text;
...
//    }
    ...
    [SWToolClass postNetworkWithUrl:@"Login.Login" andParameter:Login success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        [MBProgressHUD hideHUD];
        if (code == 0) {
            NSDictionary *infos = [info objectAtIndex:0];
            SWLiveUser *userInfo = [[SWLiveUser alloc] initWithDic:infos];
