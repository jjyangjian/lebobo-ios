...
NSString *const  wy_tx_sdkappid = @"wy_tx_sdkappid";
NSString *const  wy_beauty_code = @"beauty_code";

@implementation SWCommon
+ (void)saveProfile:(SWLiveCommon *)user
{
    ...
    [userDefaults setObject:user.tx_sdkappid forKey:wy_tx_sdkappid];
    [userDefaults setObject:user.beauty_code forKey:wy_beauty_code];
...
+ (void)clearProfile{
    ...
    [userDefaults setObject:nil forKey:wy_tx_sdkappid];
    [userDefaults setObject:nil forKey:wy_beauty_code];
    [userDefaults synchronize];
}
+ (SWLiveCommon *)myProfile{
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    SWLiveCommon *user = [[SWLiveCommon alloc] init];
    ...
    user.tx_sdkappid = [userDefaults objectForKey:wy_tx_sdkappid];
    user.beauty_code = [userDefaults objectForKey:wy_beauty_code];
...
+(NSString *)tx_sdkappid{
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSString* str = [userDefaults objectForKey: wy_tx_sdkappid];
    return str;
}
+ (NSArray *)share_type{
    return @[@"wx",@"wchat"];//@"qq",@"qzone",
}
+(BOOL)MHSDKUseState{
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSString* str = [userDefaults objectForKey:wy_beauty_code];
    if ([SWToolClass decrypt:str].length == 0) {
        return NO;
    }else{
        return YES;
    }
