...
@interface SWLiveClassViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>{
...
@implementation SWLiveClassViewController
...
- (void)requestData{
    NSString *url;
    if (_isFollow) {
        url = [NSString stringWithFormat:@"livefollow?page=%d",page];
    }else{
        url = [NSString stringWithFormat:@"classlive/%@?page=%d",_classID,page];
    }
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    SWHomeLiveModel *model = infoArray[indexPath.row];
...
//        return CGSizeMake(_window_width, _window_width/6);
//    }
//    return CGSizeMake(_window_width, _window_width/6+_window_width*0.3);
//}
