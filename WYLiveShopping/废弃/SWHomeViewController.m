...
#import "SWLiveClassViewController.h"
...
@interface SWHomeViewController ()<TYTabPagerBarDataSource,TYTabPagerBarDelegate,TYPagerControllerDataSource,TYPagerControllerDelegate,SDCycleScrollViewDelegate>{
...
@property(nonatomic, strong) SWHoverPageScrollView *mainScrollView;
...
-(SDCycleScrollView *)cycleScrollView{
    if (!_cycleScrollView) {
        ...
        _cycleScrollView.currentPageDotImage = [SWToolClass getImgWithColor:[UIColor whiteColor]];
        _cycleScrollView.pageDotImage = [SWToolClass getImgWithColor:[[UIColor whiteColor] colorWithAlphaComponent:0.3]];
...
- (void)viewDidLoad {
    ...
    self.mainScrollView = [[SWHoverPageScrollView alloc]init];
...
- (void)requestLiveClass{
    [SWToolClass getQCloudWithUrl:@"live/class" Suc:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 200) {
...
- (UIViewController *)pagerController:(TYPagerController *)pagerController controllerForIndex:(NSInteger)index prefetching:(BOOL)prefetching {
    if (index == 1) {
        SWRecommendViewController *vc = [[SWRecommendViewController alloc]init];
        WeakSelf;
        ...
        SWLiveClassViewController *VC = [[SWLiveClassViewController alloc]init];
        if (index == 0) {
            VC.isFollow = YES;
        }
